...
WebElement e1 = x.findElement(By.xpath("//button[contains(text(),'✕')]"));
		...
		WebElement e2 = x.findElement(By.xpath("//input[@placeholder='Search for products, brands and more']"));
			...
			x.manage().timeouts().implicitlyWait(Duration.ofMinutes(10));
			WebDriverWait w = new WebDriverWait(x, Duration.ofMinutes(10));
			w.until(ExpectedConditions.elementToBeClickable(e6));
			
			FluentWait<WebDriver> f = new FluentWait<WebDriver>(x).withTimeout(Duration.ofSeconds(10)).pollingEvery(Duration.ofMillis(10));
			f.until(ExpectedConditions.elementToBeClickable(e6));
}
